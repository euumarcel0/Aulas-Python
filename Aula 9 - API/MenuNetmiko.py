...
def iniciar_menu():
    while True:
        ...
        print("6 - Exibir configurações atuais do equipamento")
        ...
        if opcao == "0":
            ...
            equipamento = selecionar_equipamento()
            if equipamento == "Switch":
                print("\n", reiniciar_equipamento_cisco(switch_connection))  # Chama a função para reiniciar o switch
            elif equipamento == "Roteador":
                print("\n", reiniciar_equipamento_cisco(router_connection))  # Chama a função para reiniciar o roteador
            voltar_ao_menu_principal()
        elif opcao == "4":
...
# Função 3 para reiniciar equipamento Cisco
def reiniciar_equipamento_cisco(connection):
    try:
        output = connection.send_command_timing("reload")
        if "confirm" in output:
            output += connection.send_command_timing("yes") # Salvar informações 
            if "confirm" in output:
                time.sleep(1)  # Aguardar 1 segundo após a confirmação
                connection.send_command_timing("\n")  # Pressionar Enter após o reinício
                time.sleep(120)  # Aguardar 2 minutos após o reinício
                return {"Mensagem": "O equipamento Cisco foi reiniciado com sucesso."}
            else:
                return {"Erro": "Erro ao confirmar reinício do equipamento Cisco."}
        else:
            return {"Erro": "Erro ao reiniciar o equipamento Cisco."}
    except Exception as e:
        return {"Error": f"Erro ao reiniciar o equipamento Cisco: {e}"}

# Função 4 para enviar comando - Executa um comando e retorna o resultado
...
# Função 5 para criar usuário no equipamento Cisco
...
# Função 6 para exibir configurações atuais do equipamento Cisco
...
# Função 7 para configurar banner de mensagem no equipamento Cisco
...
def Menu_postman():
    return {
        "menu": { 
            ...
            "1": "rodar_script",
            "2": "fazer_backup",
            "3": "reiniciar_equipamento_cisco",
            "4": "enviar_comando",
            "5": "criar_usuario",
            "6": "exibir_ultimas_configuracoes",
            "7": "configurar_banner_cisco"
        }
    }
...
def executar_opcao(opcao, connection, comando=None):
    ...
    if opcao == "1":
        ...
        return reiniciar_equipamento_cisco(connection)
    elif opcao == "4":
